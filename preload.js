...
if (!fs.existsSync(dir)) {
	...
	const msg = {
		...
		defaultId: 0,
		title: 'Welcome',
		message: 'Thank you for choosing JBN Discord Rich Presence Maker !!',
...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("test").addEventListener("click", () => {
		...
		if (options.largeimage !== '') {
			...
			activity.largeImageText = "Made By JBN Technology"
		}
		...
		/*const myNotification = new Notification("JBN Discord Rich Presen	ce Maker", {
		...
		if (settings.launchedpresence == false) {
			...
			fs.writeFile(`${dir}${slash}settings.json`, JSON.stringify(settings, null, 2), 'utf8', (err) => {
				if (err) { throw err }
					...
					dialog.showMessageBox(null, msg).then(result => {
						if (result.response == 1) {
							shell.openExternal('https://github.com/JBN-Development/JBN-Discord-Rich-Presence-Maker')
						}
						if (result.response == 2) {
							shell.openExternal('https://discord.gg/d66Y3bA9CP')
						}
